...
// 1. Tính chỉ số băm để xác định danh sách cần tìm
    int idx = Hash(ht, maso); 

    // 2. Truy cập vào phần tử đầu tiên (head) của danh sách tại chỉ số đó
    Node *p = ht.table[idx].head;

    // 3. Duyệt tuần tự danh sách liên kết
    while (p != NULL) {
        // Nếu tìm thấy mã số trùng khớp
        if (p->data.Maso == maso) {
            return p; // Trả về con trỏ đến Node tìm được
        }
        p = p->next; // Chuyển sang Node tiếp theo
    }

    // 4. Trả về NULL nếu duyệt hết danh sách mà không thấy
    return NULL;
